...
module SIUnits
    ...
    typealias TypeVarUnion @compat Union{Type, TypeVar}
    quantity{m,kg,s,A,K,mol,cd,rad,sr}(T::TypeVarUnion,unit::SIUnit{m,kg,s,A,K,mol,cd,rad,sr}) = SIQuantity{T,m,kg,s,A,K,mol,cd,rad,sr}
...
quantity(T::TypeVarUnion,x::NonSIUnit) = NonSIQuantity{T,typeof(x)}
quantity(T::TypeVarUnion,U::Type{NonSIUnit}) = NonSIQuantity{T,U}
...
typealias NonSI @compat Union{NonSIQuantity,NonSIUnit}
^(x::NonSI,i::Integer) = convert(SIQuantity,x)^i
